...
const UserSelect = (props: UserSelectProps) => {
  ...
  useEffect(() => {
    store.fetchUsers();
    if (mode === 'tree') store.fetchDepartments({ withUser: true });
  }, [mode]);
  ...
  const inactiveUsers = useMemo(() => {
    return store.users?.filter(u => u.state === 0) || [];
  }, [store.users]);
  ...
  const renderInactiveUsers = (users: Console.User[]) => {
    return (
      <TreeNode
        value={`dep:-1`}
        title={'未关联部门'}
  ...
  if (mode === 'tree') {
    return (
      <TreeSelect<number>
        loading={store.departmentsLoading || store.usersLoading}
        multiple={multiple}
        onChange={handleSelect}
        treeNodeFilterProp="name"
        treeDefaultExpandAll
        value={
          store.departmentsLoading || store.usersLoading ? undefined : _value
        }
        {...restProps}
      >
        {store.depTree && renderTree(store.depTree)}
        {inactiveUsers.length > 0 && renderInactiveUsers(inactiveUsers)}
      </TreeSelect>
    );
  }
  return (
