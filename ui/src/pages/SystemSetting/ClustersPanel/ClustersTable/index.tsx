...
const ClustersTable = (props: ClustersTableProps) => {
  ...
  const { doGetClustersList, getClusterList, clusterList, doDeletedCluster } =
    useModel('systemSetting');

  const pagination = getClusterList.pagination;
        ...
        return (
          <Space>
            <Tooltip title={'编辑'}>
              <EditOutlined
                onClick={() => {
                  if (onChangeCurrentCluster && onChangeVisible && onChangeIsEditor) {
              ...
              <IconFont
                onClick={() =>
                  DeletedModal({
                    onOk: () => {
                      if (record.id)
                        doDeletedCluster.run(record.id).then(() => doGetClustersList());
                    },
  ...
  return (
    <div className={clusterPanelStyles.table}>
      <Table
        rowKey={'id'}
        loading={getClusterList.loading}
        columns={column}
        ...
        pagination={{
          ...
          onChange: (current, pageSize) => doGetClustersList({ current, pageSize }),
          responsive: true,
