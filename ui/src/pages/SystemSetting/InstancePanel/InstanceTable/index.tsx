import instanceTableStyles from '@/pages/SystemSetting/InstancePanel/InstanceTable/index.less';
...
const InstanceTable = (props: InstanceTableProps) => {
  ...
  const { doDeletedInstance, doGetInstanceList } = useModel('systemSetting');
        ...
        return (
          <Space>
            <Tooltip title={'编辑'}>
              <EditOutlined
                onClick={() => {
                  if (onChangeVisible && onChangeIsEditor && onChangeCurrentInstance) {
                  ...
                  DeletedModal(
                    {
                      onOk: () => {
                        if (record.id)
                          doDeletedInstance.run(record.id).then(() => doGetInstanceList());
                      },
