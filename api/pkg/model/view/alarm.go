...
type ReqAlarmFilterCreate struct {
	...
	SetOperatorTyp int    `json:"typ" from:"typ" binding:"required"`   // 0 default 1 INNER 2 LEFT OUTER 3 RIGHT OUTER 4 FULL OUTER 5 CROSS
	SetOperatorExp string `json:"exp" from:"exp"`                      // 操作
}

type ReqAlarmConditionCreate struct {
	SetOperatorTyp int `json:"typ" from:"typ" binding:"required"`   // 0 when 1 and  2 or
	SetOperatorExp int `json:"exp" from:"exp" binding:"required"`   // 0 avg 1 min 2 max 3 sum 4 count
