...
// 将基础字段数据保存到 data 中，基础字段优先级高于日志字段
func fillBaseFields(tid int, data []view.IndexItem) ([]view.IndexItem, error) {
	res := make([]view.IndexItem, 0)
	conds := egorm.Conds{}
	conds["tid"] = tid
	conds["kind"] = 0
	baseFields, err := db.IndexList(conds)
	if err != nil {
		return res, err
	}
	baseFieldMap := make(map[string]interface{})
	for _, b := range baseFields {
		tmp := view.IndexItem{
			Field:    b.Field,
			Alias:    b.Alias,
			Typ:      b.Typ,
		...
		res = append(res, tmp)
		baseFieldMap[tmp.Name()] = struct{}{}
	}
	for _, l := range data {
		l.Kind = 1
		if _, ok := baseFieldMap[l.Name()]; ok {
			continue
		}
		res = append(res, l)
	}
...
func AnalysisFieldsUpdate(tid int, data []view.IndexItem) (err error) {
	// 补充基础字段
	data, err = fillBaseFields(tid, data)
	if err != nil {
		return
	}
	var (
